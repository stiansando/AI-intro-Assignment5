...
class CSP:
    ...
    def check_constraints(self, var1, val1, assignment):
        neighbors = self.get_all_neighboring_arcs(var1)
        if len(neighbors) == 0:
          return True
        counter = 0
        for neighbor in neighbors:
          neighbor_values = assignment[neighbor[0]]
          all_good = False
          for neighbor_value in neighbor_values:
            if val1 != neighbor_value:
              all_good = True
          if all_good:
            counter += 1
    ...
    def backtrack(self, assignment):
        ...
        for variable in self.variables:
          valueCount += len(assignment[variable])
        if valueCount == len(self.variables):
          return assignment

        var = self.select_unassigned_variable(assignment)
        for value in assignment[var]:
          if self.check_constraints(var, value, assignment):
            assignmentTest = copy.deepcopy(assignment)
            oldValue = assignmentTest[var]
            del assignmentTest[var][:]
            assignmentTest[var].append(value)
            result = self.backtrack(assignmentTest)
            if result != False:
              return result
            assignmentTest[var] = oldValue
        return False
